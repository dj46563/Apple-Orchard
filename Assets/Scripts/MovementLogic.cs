...
public class MovementLogic
{
    ...
    public static Vector3 CalculateDelta(InputCompressor.Inputs inputs, Quaternion rotation, Vector3 pastDelta, bool grounded)
    {
        int horizontal = (inputs.D ? 1 : 0) - (inputs.A ? 1 : 0);
        int vertical = (inputs.W ? 1 : 0) - (inputs.S ? 1 : 0);

        float yDelta = pastDelta.y;
        if (!grounded)
        ...
        Vector3 XAndZ = yRotation * new Vector3(horizontal, 0, vertical) * Speed;
